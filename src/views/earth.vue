<template>
  <div class="three-box-ct">
    <div class="mgb-10">
      <el-button type="primary" @click="stop">显示按钮</el-button>
    </div>
    <div class="three-box">
      <canvas width="500" height="500" id="thr" ref="thr"></canvas>
    </div>
  </div>
</template>
...
import mapJPG from '@/assets/map2.jpg';
import mapJPG2 from '@/assets/map.jpg';
export default {
  props: [''],
  data() {
    return {
  ...
  methods: {
    ...
    drawEarth() {
      ...
      const texture = textureLoader.load(mapJPG, (texture) => {
    ...
    //   textureLoader.load(mapJPG2, (texture) => {
    //     const material = new THREE.MeshLambertMaterial({
    //       map: texture,
    //       transparent: true,
    //       opacity: 0.3,
    ...
    sport(curvePoints, index) {
      ...
      console.log(index, this.destination.length, 'fsadf');
    ...
    getBezierPoint(v0, v3) {
      ...
      // 顶点坐标
      let vtop = new THREE.Vector3(0, 0, 0);
      rayLine.at(1, vtop)
      rayLine.at( hLen/vtop.distanceTo(p0), vtop); // 几倍位置
      // 控制点坐标
    ...
    // 经纬度转化为坐标 !! three.js在113版本中，将Math改了MathUtils
